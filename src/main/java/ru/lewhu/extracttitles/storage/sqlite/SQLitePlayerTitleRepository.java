...
public final class SQLitePlayerTitleRepository implements PlayerTitleRepository {
    private final SQLiteStorage storage;
    private final ExecutorService executor = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "extracttitles-sqlite");
        t.setDaemon(true);
        return t;
    });

    public SQLitePlayerTitleRepository(SQLiteStorage storage) {
    ...
    private void saveProfile(PlayerTitleProfile profile, Connection connection) throws SQLException {
        String upsert = """
                ...
                VALUES(?, ?, ?)
                ON CONFLICT(uuid) DO UPDATE SET active_title = excluded.active_title, updated_at = excluded.updated_at
                """;
